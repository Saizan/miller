...
mutual
  ...
  sub-nat {b1} {false} {f = f} {i} (mvar u xs) = begin 
    ...
    replace (f _ u) (rens i (subs f xs))              ≡⟨ eval-ext (f _ u) 
       (λ x → transd _ (evals-ø (subs f xs) i (idEnv-≤[] i) x) (evals-nat (subs f xs) reflA i x)) ⟩
...
mutual
  ...
  eval-nats {_} {true}  {f = f} {g1} {g2} g gr (mvar u j)  = begin 
    ...
    eval (f _ u) (evals (reifys (build (λ z → get g2 (j $ z)))) idEnv)
        ≡⟨ eval-ext (f _ u) (evals-o idEnv (precompose-∘ idEnv gr j)) ⟩
    eval (f _ u) (build (λ z → get g2 (j $ z))) 
        ≡⟨ sym (eval-ø (f _ u) j (λ x → ≡-d (get-build (λ v → get g2 (j $ v)) x))) ⟩
    eval (sub f (mvar u j)) g2 ∎  
    ...
    nf (f _ u) (evals (reifys (evals (subs f ts) g2)) idEnv) ≡⟨ sym
                                                                  (nf-∘ (f _ u) g2
                                                                   (λ {S} x →
                                                                      ...
                                                                      (symd _ (evals-o idEnv (evals-∘ idEnv gr (subs f ts)) x)))) ⟩
