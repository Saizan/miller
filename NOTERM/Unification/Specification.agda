module NOTERM.Unification.Specification where
...
Spec x y = ∃σ-pat Max (Unifies x y) ⊎ ¬ ∃σ Unifies x y
...
∃σMax[Unifies[x,x]] : ∀ {Sg G D T} (x : Term Sg G D T) -> ∃σ-pat Max (Unifies x x)
