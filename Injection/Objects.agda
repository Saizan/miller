...
ext-∘ f g f1 g1 eq = ext-$ _ _ (\ x v -> trans (apply-∘ _ _) (trans (eq x v) (sym (apply-∘ _ _))))
abstract
  ...
  e$u≡m equ a m eq =  
    let
        open Equalizer equ
        [m] = m ∷ [] [ _ ]
        u = universal [m] (ext-∘ _ _ _ _ (λ {.a zero → eq; x (suc ())}))
  ...
  p$u≡q f g p a q₂ q₁ eq = 
   let
    ...
    [q₂] = q₂ ∷ [] [ _ ]
    u : Inj (a ∷ []) P
    u = universal [q₁] [q₂] (ext-∘ _ _ _ _ (λ {._ zero → eq; _ (suc ())}))
   in u $ zero , 
...
_→zero∷e⇒i∷f,i∷g {a = a} {v = u} {f} {g} {pf} {pg} {E} {e} equ = 
   record {
     commutes = ext-∘ _ _ _ _ aux;
     universal = λ m commutes₁ → proj₁ (uni m commutes₁);
     universal-unique = λ u₁ e∘u≡m → ∘i-inj (zero ∷[] e) u₁ _ (trans e∘u≡m (sym (proj₂ (uni _ _))));
...
_,_→weake⇒i∷f,j∷g {a = a} {u = u}{v} u≢v {f} {g} {pf} {pg} {E} {e} equ = 
       record {
         commutes = ext-∘ _ _ _ _ aux;
         universal = λ m commutes₁ → proj₁ (uni m commutes₁);
         universal-unique = λ u₁ e∘u≡m → ∘i-inj (weak e) u₁ _ (trans e∘u≡m (sym (proj₂ (uni _ _))));
         e∘universal≡m = \ {Q} {m} {comm} -> proj₂ (uni m comm) }
 where 
   open IsEqualizer equ
...
∈-pullback {A}{X}{Y}{Z}{P}{f = f} {g} {p₁}{p₂}pull{a}{x}{pf}{x∉p₁}  = record {
                    ...
                    universal-unique = λ u p₁∘u≡q₁ p₂∘u≡q₂ → ∘i-inj (x ∷ p₁ [ x∉p₁ ]) u (proj₁ (uni _ _ _)) (trans p₁∘u≡q₁ (proj₁ (proj₂ (uni _ _ _))));
  ...
  where
    ...
    aux : ∀ t (v : _ ∋ t) -> f $ ((x ∷ p₁ [ x∉p₁ ]) $ v) ≡
      ...
      ((zero ∷[] p₂) $ v) 
    aux .a zero = refl
    aux t (suc v) = trans (trans (sym (apply-∘ _ _)) (trans (cong (λ f₁ → f₁ $ v) 
                          (IsPullback.commutes pull)) (apply-∘ _ _))) (cong (_$_ ((f $ x) ∷ g [ pf ])) (sym (Inj-thin-$ zero p₂ v)))
...
∉-pullback {A}{X}{Y}{Z}{f = f} {g = g} {p₁}{p₂}pull {a} {i} {pf} i∉f = record {
                        commutes = trans commutes (ext-∘ _ _ _ _ (λ x v →  
  ...
  where
    ...
    x∉p₁ = ∉Im-∉ p₁ x λ b x≡k$b → ∉-∉Im g (f $ x) pf (p₂ $ b) 
              (begin f $ x        ≡⟨ cong (_$_ f) x≡k$b ⟩ 
                     f $ (p₁ $ b) ≡⟨ trans (sym (apply-∘ _ _)) (trans (cong (λ f₁ → f₁ $ b) commutes) (apply-∘ _ _)) ⟩ 
                     g $ (p₂ $ b) ∎)
