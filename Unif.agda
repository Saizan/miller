...
mutual
  ...
  check u (con c ts) = Data.Sum.map (con c) (map-occ (con c)) (checks u ts) 
  check u (fun w j) with thick u w
  ... | inj₁ (z , eq) = inj₁ (fun z j eq)
  check u (fun .u j) | inj₂ refl = inj₂ (_ , (j , ([] , refl)))
  check u (var x ts) = Data.Sum.map (var x) (map-occ (var x)) (checks u ts)
  check u (lam t) = Data.Sum.map lam (map-occ lam) (check u t)
  
  checks : ∀ {Sg G D Ts S} (u : G ∋ S) (ts : Tms Sg G D Ts) → MRTms Sg (G - u) D G (thin u) Ts ts ⊎ u OccursIn ts
  checks u [] = inj₁ []
