...
mutual
  ...
  uni-pullbackT i j h k uni (con c ts) (con .c ts₁) (refl , refl , eqts) = (con c (proj₁ recs)) , (cong (con c) (proj₂ recs))
  ...
  uni-pullbackT i j h k uni (fun u j₁) (fun .u j₂) (refl , refl , eq) = fun u (proj₁ rec) , cong (fun u) (sym (proj₂ prec)) 
  ...
  uni-pullbackT i j h k uni (var x ts) (var x₁ ts₁) (refl , ix≡jx₁ , eq) = 
                (var (proj₁ rec) (proj₁ recs)) , ((cong₂ var (proj₂ (proj₂ rec)) (proj₂ recs)))
             where rec = (uni _ x₁ x ix≡jx₁)
                   recs = uni-pullbackTs i j h k uni ts ts₁ eq
  uni-pullbackT i j h k uni (lam t) (lam s) eq = lam (proj₁ rec) , cong lam prec where
...
mutual
  ...
  purge-gen i (con c ts) s (con c₁ ts₁) (refl , _ , eq) = purge-gens i ts s ts₁ eq
  purge-gen i (con c ts) s (fun u j) ()
  purge-gen i (con c ts) s (var x ts₁) ()
  purge-gen {Sg} {G} i (fun {S = S} u j) {G2} s z eq with (purje i j) 
  ... | (P , k , h , pull , uni) = r , proof where
            uniT = uni-pullbackT _ _ k h uni z (s S u) eq
            r : (S₁ : MTy) → type S <<- P ∷ G - u ∋ S₁ → Tm Sg G2 (ctx S₁) ([] ->> type S₁)
            r .(type S <<- P) zero = proj₁ uniT
  ...
  purge-gen i (var x ts) s (fun {type <<- ctx} u j) ()
  purge-gen i (var x ts) s (var x₁ ts₁) (refl , eqv , eq) = purge-gens i ts s ts₁ eq
  purge-gen i (lam t) s (lam z) eq = purge-gen (cons i) t s z eq
  ...
  ... | (r , s≡r∘ρ) with purge-gens i (subs (toSub (proj₁ (proj₂ (purge i t)))) ts) r zs (≡-T (trans (proj₂ (eqTms (rens i zs) (subs s ts)) eqts) 
